...
title_str = sprintf('Bias Type : - -- --- ---- ----- , Total Length : %5.3f , NoD : %d , Bias Factor : %5.3f , Max-Size : %5.3f , Min-Size : %5.3f',L,NoD,B,max(M),min(M));
title(title_str)
xlim([0 L])
xline(C)
...
fprintf(fid,'|%-30s||%-30s||%-30s||%-30s|\n\n','Element No','Length','Start Point','End Point');
fprintf(fid,'|%-30d||%-30.20f||%-30.20f||%-30.20f|\n',transpose(Table));
fclose(fid);